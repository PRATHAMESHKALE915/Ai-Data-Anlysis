...
async function setupApp() {
  ...
  // ── Security Headers & Rate Limiting (ECC Audit & Gitleaks Checklist) ───
  app.use(
    helmet({
      contentSecurityPolicy: false, // Allowed for inline React Vite dev server
      crossOriginEmbedderPolicy: false,
    })
  );

  // Rate limiter for general API routes (100 requests per 15 minutes per IP)
  ...
  // Strict rate limiter for AI analysis endpoints (15 requests per 15 minutes per IP)
  ...
  // Cloud SQL & Firebase DB endpoints
  ...
  app.get("/api/db/datasets", requireAuth, async (req: AuthRequest, res) => {
    try {
      if (!req.user) return res.status(401).json({ error: "Unauthorized" });
      if (!process.env.SQL_HOST) {
  ...
  app.post("/api/db/datasets", requireAuth, async (req: AuthRequest, res) => {
    try {
      if (!req.user) return res.status(401).json({ error: "Unauthorized" });
  ...
  app.get("/api/db/reports", requireAuth, async (req: AuthRequest, res) => {
    try {
      if (!req.user) return res.status(401).json({ error: "Unauthorized" });
  ...
  app.post("/api/db/reports", requireAuth, async (req: AuthRequest, res) => {
    try {
      if (!req.user) return res.status(401).json({ error: "Unauthorized" });
  ...
  app.get("/api/db/routines", requireAuth, async (req: AuthRequest, res) => {
    try {
      if (!req.user) return res.status(401).json({ error: "Unauthorized" });
  ...
  app.post("/api/db/routines", requireAuth, async (req: AuthRequest, res) => {
    try {
      if (!req.user) return res.status(401).json({ error: "Unauthorized" });
  ...
  app.post("/api/convert", upload.array("file"), async (req: any, res: any) => {
    try {
    ...
    if (!apiKey || apiKey.trim() === "" || apiKey === "your_gemini_api_key_here") {
      return res.status(400).json({
        error:
          ...
          "Open the file '.env.local' in your project root and set GEMINI_API_KEY to your key. " +
...
`;
      ...
      if (!response.ok) {
        ...
        const isAuthError =
          ...
          errorText.toLowerCase().includes("unregistered callers") ||
          errorText.toLowerCase().includes("api key") ||
          displayMessage.toLowerCase().includes("permission_denied");
      ...
      const server = app
        ...
        .on("error", (err: any) => {
          if (err.code === "EADDRINUSE") {
            ...
            setTimeout(() => startListening(port), 1000);
          } else {
            console.error(err);
          }
        });
